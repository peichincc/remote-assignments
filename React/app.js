...
window.onload = () => {
  ...
  backToIndex();
  // Check the URL paramenter and get data
  ...
  if (window.location.href.indexOf("women") !== -1) {
    getProductData(`https://api.appworks-school.tw/api/1.0/products/women`);
  } else if (window.location.href.indexOf("men") !== -1) {
    getProductData(`https://api.appworks-school.tw/api/1.0/products/men`);
    ...
    let urlSearchValue = searchParams.get("keyword");
    getProductData(
      `https://api.appworks-school.tw/api/1.0/products/search?keyword=` +
        urlSearchValue
    );
  } else {
    getProductData(`https://api.appworks-school.tw/api/1.0/products/all`);
  }
  if (!category) {
    category = "all";
    changeTarget = "all";
  } else {
    changeTarget = `nav-${category}`;
  }
...
function dataToDOM(slideObjList) {
  const slidesHtml = slideObjList
    .map(({ id, product_id, picture, story }, index) => {
      ...
      const author = text[3];
      return `
          <a href="product.html?id=${product_id}"><div class="slideWelcomeImage" id="data-slide-${id}">
  ...
  const productHtml = productObjList
    .map((data) => {
      ...
      return `<div class="box">
                <div class="box-image"><a href="product.html?id=${data.id}"><img src=${data.main_image}></a></div>
...
// To update the cart number
const cartNumberIndex = document.getElementsByClassName("cartnumber_in");
// console.log(cartNumberIndex);
if (localStorage.userSelectData) {
  ...
  console.log("Current Cartnumber:", sum);
  cartNumberIndex[0].innerText = sum;
  cartNumberIndex[1].innerText = sum;
}
